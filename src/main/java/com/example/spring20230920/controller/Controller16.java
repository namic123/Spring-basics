...
public class Controller16 {
    ...
    public void method1(Model model, HttpSession session){
        ...
        session.setAttribute("attr1", "윷놓이");
        application.setAttribute("attr1", "토끼");

    }
    @GetMapping("sub2")
    public void method2(Model model, HttpSession session){
    ...
    public void method3(Model model, HttpSession session){
        model.addAttribute("modelAttr1", "보륾달");
        session.setAttribute("sessionAttr1", "보륾달");
    ...
    @GetMapping("sub11")
    public void method10(HttpSession session){
    ...
    public String method14( RedirectAttributes rttr){
        rttr.addFlashAttribute("foodList", List.of("피자", "치킨"));
    ...
    rttr.addAttribute("address","쿼리스트링에 붙음");

        System.out.println("rttr.getAttribute(\"attr1\") = " + rttr.getAttribute("attr1"));
